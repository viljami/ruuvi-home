...
# Import ruuvitag-sensor for testing compatibility
...
class TestRuuviDataFormat:
    ...
    def test_data_hex_structure(self):
        ...
        # Decode temperature (bytes 1-2, signed 16-bit integer, 0.005°C resolution)
        temp_raw = struct.unpack('>h', data_bytes[1:3])[0]
        temp = temp_raw * 0.005
        assert abs(temp - 21.5) < 0.01, "Temperature decoding failed"
        
        # Decode humidity (bytes 3-4, unsigned 16-bit integer, 0.0025% resolution)
        ...
        # Decode pressure (bytes 5-6, unsigned 16-bit integer, +50000 Pa offset)
    ...
    def test_temperature_range(self):
        ...
        for temp in [-30.0, -10.0, 0.0, 10.0, 20.0, 50.0, 80.0]:
            ...
            assert abs(decoded_temp - temp) < 0.01, f"Temperature {temp}°C not encoded/decoded correctly"

    def test_ruuvitag_sensor_compatibility(self):
        """Test that our encoded data is compatible with ruuvitag-sensor decoder"""
        test_mac = "C6:99:AB:11:22:33"
        ...
        assert abs(sensor_data['temperature'] - 23.45) < 0.1, "Temperature mismatch"
        assert abs(sensor_data['humidity'] - 65.25) < 0.1, "Humidity mismatch"
        assert abs(sensor_data['pressure'] - 1013.25) < 1, "Pressure mismatch"  # hPa
        
        # Verify MAC address
        expected_mac = test_mac.replace(':', '').lower()
        assert sensor_data['mac'] == expected_mac, f"MAC mismatch: {sensor_data['mac']} != {expected_mac}"
        
        # Verify acceleration values
        assert abs(sensor_data['acceleration_x'] - 100) < 5, "Acceleration X mismatch"  # mG
        assert abs(sensor_data['acceleration_y'] - (-50)) < 5, "Acceleration Y mismatch"  # mG
        assert abs(sensor_data['acceleration_z'] - 980) < 10, "Acceleration Z mismatch"  # mG
...
class TestGatewayMessageGeneration:
    ...
    def test_gateway_message_structure(self):
        """Test that the generated gateway message has the correct structure"""
        test_mac = "C6:99:AB:11:22:33"
        ...
        with patch.object(simulator, 'GATEWAY_MAC', "AA:BB:CC:DD:EE:FF"):
            ...
            assert all(c in "0123456789ABCDEF" for c in gateway_message["data"])
            ...
            now = int(time.time())
            assert abs(gateway_message["ts"] - now) < 86400  # Within 24 hours
    ...
    def test_rssi_values_are_realistic(self):
        ...
        # Check ranges
        assert all(-90 <= rssi <= -60 for rssi in rssi_values), "RSSI outside realistic range"
    ...
    def test_matches_example_format(self):
        ...
        for key in example:
            ...
            assert type(gateway_message[key]) == type(example[key]), f"Type mismatch for field: {key}"
    ...
    def test_sensor_mac_consistency(self):
        ...
        for test_mac in test_macs:
            ...
            messages = [simulator.generate_ruuvi_gateway_message(test_mac) for _ in range(3)]
            
            for msg in messages:
                data_hex = msg["data"]
                # Extract sensor payload (skip BLE headers)
                payload_start = 14  # Skip: 020106 (6) + 1BFF990405 (8) = 14 chars
                ...
                expected_mac = test_mac.replace(':', '').lower()
                assert sensor_data['mac'] == expected_mac, \
                    f"MAC mismatch for {test_mac}: got {sensor_data['mac']}"
...
class TestMQTTIntegration:
    ...
    def test_mqtt_client_connection(self, mqtt_client_mock):
        ...
        with patch('sys.argv', ['simulator.py']):
            # Run the main function with the mocked client, but stop before the infinite loop
