...
# Generate environment file using shared configuration
generate_env_file() {
    local context="$MODULE_CONTEXT"
    local env_path="$PROJECT_DIR/.env"

    log_info "$context" "Generating environment file using shared configuration"

    # Initialize shared configuration to get all derived values
    initialize_configuration

    cat > "$env_path" << EOF
# Ruuvi Home Environment Configuration
# Generated by setup script using shared configuration library
...
AUTH_DATABASE_URL=$(construct_auth_database_url)
...
MQTT_BROKER_URL=$(construct_mqtt_url)
...
# Monitoring Configuration (using shared defaults)
...
EOF
    ...
    log_success "$context" "Environment file generated using shared configuration: $env_path"
}
